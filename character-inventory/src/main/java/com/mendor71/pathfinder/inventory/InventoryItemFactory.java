...
public class InventoryItemFactory {
    ...
    private InventoryItemFactory() throws IOException {
        InputStream isBase = getClass().getClassLoader().getResourceAsStream("src/main/resources/inventory_items.json");
        List<InventoryItem> baseItems = loadFromSource(isBase);
        baseItems.forEach(v -> inventoryItemStorage.put(v.getName(), v));

        try {
            InputStream isAdditional = new FileInputStream(new File("src/main/resources/inventory_items.json"));
